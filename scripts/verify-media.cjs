...
(async()=>{
 ...
 for(const original of originals.sort()){
  const file='videos/'+path.basename(original,path.extname(original))+'-4k30.mp4';
  const output=path.join(root,'dist',file),a=probe(original),b=probe(output);
  if(b.streams[0].width!==3840||b.streams[0].height!==2160||b.streams[0].r_frame_rate!=='30/1')throw Error(file+' 必須為4K 30fps');
 ...
 fs.writeFileSync(path.join(root,'media.json'),JSON.stringify({releaseTag:'survey-media-v4',encoding:'H.264 4K 30fps CRF20, AAC 128k, faststart',videos},null,2)+'\n');
 console.log(JSON.stringify({videos:videos.length,bytes:videos.reduce((n,v)=>n+v.bytes,0),verified:'4K, 30fps, duration, SHA256'}));
